...
export const getLegalPawnMoves = (
	...
	const hasPawnMoved =
		side === 'white'
			? square.index <= 55 && square.index >= 48
			: square.index <= 15 && square.index >= 8;
	...
	const isOnEnPassantEnemySquare =
		side === 'white'
			? square.index >= 24 && square.index <= 31
			: square.index >= 32 && square.index <= 39;
	...
	if (
		(leftCaptureInfo?.side !== side && leftCaptureInfo?.piece) ||
		(enPassant?.index && enPassant.index === leftCapture && isOnEnPassantEnemySquare)
	) {
		legalMoves.push(getFullSquareInfo(leftCapture));
	}

	if (
		(rightCaptureInfo?.side !== side && rightCaptureInfo?.piece) ||
		(enPassant?.index && enPassant.index === rightCapture && isOnEnPassantEnemySquare)
	) {
