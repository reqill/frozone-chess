...
export const getLegalKnightMoves = (gameObject: FenNotationObjectType, pieceInfo: PiecePositionInfoType) => {
    ...
    const canPerformShortUpLeftMove = square.index - 8 >= 0 && square.index % 8 !== 0 && square.index % 8 !== 1;
    const canPerformLongUpRightMove = square.index - 16 >= 0 && square.index % 8 !== 7;
    const canPerformShortUpRightMove = square.index - 8 >= 0 && square.index % 8 !== 6 && square.index % 8 !== 7;

    const canPerformLongDownLeftMove = square.index + 16 <= 63 && square.index % 8 !== 0;
    const canPerformShortDownLeftMove = square.index + 8 <= 63 && square.index % 8 !== 0 && square.index % 8 !== 1 ;
    const canPerformLongDownRightMove = square.index + 16 <= 63 && square.index % 8 !== 7 ;
    const canPerformShortDownRightMove = square.index + 8 <= 63 && square.index % 8 !== 6 && square.index % 8 !== 7;
